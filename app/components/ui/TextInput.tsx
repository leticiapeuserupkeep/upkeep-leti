  ...
  ({ label, suffix, error, errorMessage, className = '', id, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-[var(--space-xs)]">
        ...
        <div className={`flex items-center h-10 rounded-[var(--radius-lg)] border overflow-hidden transition-colors duration-[var(--duration-fast)] ${error ? 'border-[var(--color-error)] focus-within:border-[var(--color-error)] shadow-[0_0_1px_3px_rgba(206,44,49,0.2)]' : 'border-[var(--border-default)] focus-within:border-[var(--color-accent-7)] focus-within:shadow-[0_0_1px_3px_rgba(0,106,220,0.1)]'}`}>
          <input
            ref={ref}
            id={inputId}
    ...
    return (
      <div className="flex flex-col gap-[var(--space-xs)]">
        ...
        <textarea
          ref={ref}
          id={inputId}
          className={`px-[var(--space-md)] py-[var(--space-sm)] rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--surface-primary)] text-[length:var(--font-size-sm)] text-[var(--color-neutral-11)] outline-none placeholder:text-[var(--color-neutral-7)] focus:border-[var(--color-accent-7)] focus:shadow-[0_0_1px_3px_rgba(0,106,220,0.1)] transition-colors duration-[var(--duration-fast)] resize-y min-h-[80px] disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
          {...props}
        />
      </div>
    )
